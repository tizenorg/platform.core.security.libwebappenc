...
int wae_tc_get_alias()
{
    ...
    FPRINTF("...pkgid=%s, alias=%s, system alias=%s\n", pkgId, alias, sys_alias);
...
int wae_tc_initail_value()
{
    ...
    // TODO
    //sFPRINTF(path, "%s/%s_%s.xml", KEY_MANAGER_INITIAL_VALUE_DIR, KEY_MANAGER_INITIAL_VALUE_FILE_PFX, pkgId);
    unlink(path);
    ...
    if(ret != WAE_ERROR_NONE) {
        FPRINTF("...FAIL: _write_initial_value_file.\n");
        goto error;
    }
    ...
    if(ret != WAE_ERROR_NONE) {
        FPRINTF("...FAIL: _read_initial_value_file.\n");
        goto error;
    }

    FPRINTF("...ORIGINAL PKG_ID:%s\n", pkgId);
    FPRINTF("...READ     PKG_ID:%s\n", readPkgId);
    if(strcmp(pkgId, readPkgId) != 0) {
        FPRINTF("...FAIL: different pkgid. orig=%s, read=%s.\n", pkgId, readPkgId);
        ret = WAE_ERROR_FILE;
        goto error;
    }
