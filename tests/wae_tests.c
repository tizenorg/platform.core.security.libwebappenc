...
int wae_tc_encrypt_decrypt_app_dek()
{
    ...
    int encryptedDekLen = 0;
    unsigned char* decryptedDek = NULL;
    int decryptedDekLen = 0;
...
int wae_tc_get_preloaded_app_dek_file_path()
{
    ...
    const char *expectedPath = "/usr/share/wae/app_dek/WAE_APP_DEK_test_pkg.adek";
    char path[100];
...
int wae_tc_encrypt_decrypt_web_application()
{
    ...
    ret = wae_encrypt_web_application(pkgId1, (const unsigned char*)plaintext, plaintextLen, 
                                      &encrypted, &encLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
    ...
    ret = wae_decrypt_web_application(pkgId1, encrypted, encLen, &decrypted, &decLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
    ...
    ret = wae_encrypt_web_application(pkgId2, (const unsigned char*)plaintext, plaintextLen,
                                      &encrypted, &encLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...FAIL: wae_encrypt_web_application. ret=%d\n", ret);
        goto error;
    ...
    ret = wae_decrypt_web_application(pkgId2, encrypted, encLen, &decrypted, &decLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...FAIL: wae_decrypt_web_application. ret=%d\n", ret);
        goto error;
    }
...
const char* PRELOADED_WAPP_PKGID     = "testpkg_for_preloaded_offline";
const char* PRELOADED_WAPP_TEST_DATA = "adbdfdfdfdfdererfdfdfererfdrerfdrer";
const char* PRELOADED_WAPP_FILE1     = "/usr/share/wae/test/PRELOADED_WAPP_FILE1.enc";
const char* PRELOADED_WAPP_FILE2     = "/usr/share/wae/test/PRELOADED_WAPP_FILE2.enc";

int encrypt_preloaded_app()
{
    int ret = WAE_ERROR_NONE;

    const char* pkgid = PRELOADED_WAPP_PKGID;
    const char* plaintext= PRELOADED_WAPP_TEST_DATA;
    int plaintextLen = strlen(plaintext);
    unsigned char* encrypted = NULL;
    int encLen = 0;
    int isPreloaded = 1; // test for preloaded web application

    // check if files already exist.
    ret = _read_from_file(PRELOADED_WAPP_FILE1, &encrypted, &encLen);
    if(ret == WAE_ERROR_NONE){
        FPRINTF("...FAIL: File already exist. file=%s\n", PRELOADED_WAPP_FILE1);
        goto error;
    }
    ret = _read_from_file(PRELOADED_WAPP_FILE2, &encrypted, &encLen);
    if(ret == WAE_ERROR_NONE){
        FPRINTF("...FAIL: File already exist. file=%s\n", PRELOADED_WAPP_FILE2);
        goto error;
    }
    

    ret = wae_encrypt_web_application(pkgid, (const unsigned char*)plaintext, plaintextLen,
                                      &encrypted, &encLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...PRELOADED_APP_ENC:FAIL: wae_encrypt_web_application. ret=%d\n", ret);
        goto error;
    }
    // write a file1 
    ret = _write_to_file(PRELOADED_WAPP_FILE1, encrypted, encLen);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...PRELOADED_APP_ENC:FAIL: _write_to_file. ret=%d\n", ret);
        goto error;
    }
    ...
    ret = wae_encrypt_web_application(pkgid, (const unsigned char*)plaintext, plaintextLen,
                                      &encrypted, &encLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...PRELOADED_APP_ENC:FAIL: wae_encrypt_web_application2. ret=%d\n", ret);
        goto error;
    }
    // write a file2 
    ret = _write_to_file(PRELOADED_WAPP_FILE2, encrypted, encLen);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...PRELOADED_APP_ENC:FAIL: _write_to_file2. ret=%d\n", ret);
        goto error;
    }
error:
    if(encrypted != NULL)
    ...
    if(ret == WAE_ERROR_NONE)
        FPRINTF("...PRELOADED_APP_ENC:SUCCESS: encrypt_preloaded_app.\n");
    else
        FPRINTF("...PRELOADED_APP_ENC:FAIL: encrypt_preloaded_app. \n");
    return ret;
}

int wae_tc_decrypt_preloaded_app()
{
    int ret = WAE_ERROR_NONE;
    ...
    ret = _read_from_file(PRELOADED_WAPP_FILE1, &encrypted, &encLen);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...FAIL: _read_from_file. ret=%d\n", ret);
        goto error;
    }

    ret = wae_decrypt_web_application(pkgid, encrypted, encLen, &decrypted, &decLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        FPRINTF("...FAIL: wae_decrypt_web_application. ret=%d\n", ret);
        goto error;
    }
    ...
    if(strcmp(plaintext, (char*) decrypted) != 0) {
        FPRINTF("...FAIL: the decrypted value is not same with the plain text. plain text=%s, decrypted=%s\n",
...
int run_test_cases()
{
    ...
    //RUNTC(wae_tc_decrypt_preloaded_app, "wae_tc_decrypt_preloaded_app");
...
int main(int argc, char *argv[]) 
{
    ...
    if(argc <= 1) {
        ret = run_test_cases();
    } else if(strcmp(argv[1],"--enc_preloaded_app") == 0) {
        ret = encrypt_preloaded_app();
        if(ret == WAE_ERROR_NONE)
            ret = 0;
        else
            ret = -1;
    } else { // invalid argument
        FPRINTF("...FAIL: invalid argement. arg=%s\n", argv[1]);
        ret = -1;
    }
