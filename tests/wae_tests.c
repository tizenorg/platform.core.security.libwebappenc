...
int wae_tc_encrypt_decrypt_app_dek()
{
    ...
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: encrypt_app_dek. ret=%d\n", ret);
        ret = WAE_ERROR_CRYPTO;
        goto error;
    }

    ret = decrypt_app_dek((const unsigned char*) priKey, strlen(priKey),
                          encryptedDek, encryptedDekLen, &decryptedDek, &decryptedDekLen);
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: decrypt_app_dek. ret=%d\n", ret);
        ret = WAE_ERROR_CRYPTO;
        goto error;
    }
    ...
    if(_compare_binary(dek, sizeof(dek), decryptedDek, decryptedDekLen) != 0) {
        printf("...FAIL: ORIG DEK != decrypted DEK\n");
        ret = WAE_ERROR_CRYPTO;
        goto error;
    } 
...
int wae_tc_cache()
{
    ...
    if(retDek == NULL || _compare_binary(dek1, 32, retDek, 32) != 0) {
        printf("failed in cache. Diffent DEK1\n");  
    ...
    if(retDek == NULL || _compare_binary(dek2, 32, retDek, 32) != 0) {
        printf("failed in cache. Diffent DEK2\n");  
        ret = WAE_ERROR_UNKNOWN;
        goto error;
    } 
    ...
    if(retDek == NULL || _compare_binary(dek3, 32, retDek, 32) != 0) {
        printf("failed in cache. Diffent DEK3\n");  
        ret = WAE_ERROR_UNKNOWN;
        goto error;
    } 
    ...
    if(retDek != NULL) {
        printf("failed in cache. Wrong DEK_DUMMY1 returned\n");  
    ...
    retDek = _get_app_dek_from_cache(pkg3);
    if(retDek != NULL) {
        printf("fail to remove app dek from cache\n");
        ret = WAE_ERROR_UNKNOWN;
        goto error;
    }
    ...
    retDek = _get_app_dek_from_cache(pkg2);
    if(retDek != NULL) {
        printf("failed in cache. Wrong DEK_DUMMY2 returned\n");  
        _print_binary_to_hex("retured wrong DEK : ", retDek, 32);
        ret = WAE_ERROR_UNKNOWN;
        goto error;
    }
...
int wae_tc_extract_pkg_id_from_file_name()
{
    ...
    printf("...expected pkgId: %s\n", expectedPkgId);
    printf("...returned pkgId: %s\n", pkgId);
...
int wae_tc_read_write_encrypted_app_dek()
{
    ...
    if(ret != WAE_ERROR_NONE) {
        printf("Fail to _write_encrypted_app_dek_to_file. pkgId=%s\n", pkgId);
        goto error;
    }

    ret = _read_encrypted_app_dek_from_file(pkgId, &readDek, &readDekLen);
    if(ret != WAE_ERROR_NONE) {
        printf("Fail to _read_encrypted_app_dek_from_file. pkgId=%s\n", pkgId);
        goto error;
    }
...
int wae_tc_encrypt_decrypt_web_application()
{
    ...
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: wae_encrypt_web_application. ret=%d\n", ret);
        goto error;
    }

    ret = wae_decrypt_web_application(pkgId1, encrypted, encLen, &decrypted, &decLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: wae_decrypt_web_application. ret=%d\n", ret);
        goto error;
    }
    ...
    memcpy(decrypted_str, decrypted, decLen);
    printf("...plaintext(downloaded) = %s\n", plaintext);
    printf("...decrypted(downloaded) = %s\n", decrypted_str);
    if(strcmp(plaintext, decrypted_str) != 0) {
        printf("...FAIL: plaintext(%s) != decrypted(%s)\n", plaintext, decrypted_str);
        ret = WAE_ERROR_CRYPTO;
        goto error;
    ...
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: wae_remove_app_dek. ret=%d\n", ret);
        goto error;
    }
    ...
    ret = wae_encrypt_web_application(pkgId2, (const unsigned char*)plaintext, plaintextLen,
                                      &encrypted, &encLen, isPreloaded);
    if(ret != WAE_ERROR_NONE){
        printf("...FAIL: wae_encrypt_web_application2. ret=%d\n", ret);
        goto error;
    }
    ...
    memcpy(decrypted_str, decrypted, decLen);
    printf("...plaintext(preloaded) = %s\n", plaintext);
    printf("...decrypted(preloaded) = %s\n", decrypted_str);
    if(strcmp(plaintext, decrypted_str) != 0) {
        printf("...FAIL: plaintext(%s) != decrypted(%s)\n", plaintext, decrypted_str);
        ret = WAE_ERROR_CRYPTO;
        goto error;
