...
int _write_encrypted_app_dek_to_file(const char* pPkgId, const unsigned char* encrypted_app_dek, int len)
{
    ...
    write_len = fwrite(encrypted_app_dek, 1, len, f);
    if(write_len != len) {
...
int load_preloaded_app_deks()
{
    ...
    ret = get_app_deks_loaded(); 
    if(ret == WAE_ERROR_KEY_EXISTS) {
        return WAE_ERROR_NONE;
    }

    // TODO : read from key-manager
    ret = _read_from_file(APP_DEK_KEK_PRI_KEY_PATH, &priKey, &priKeyLen);
    if(ret != WAE_ERROR_NONE) {
    ...
    for(;;) {
        ...
        if(entry.d_type == DT_REG && strstr(entry.d_name, APP_DEK_FILE_PFX) != NULL) { 
            ...
            ret = decrypt_app_dek(priKey, priKeyLen, encrypted_app_dek, encrypted_app_dek_len, 
                                  &app_dek, &app_dek_len);
