...
int get_old_duk(const char *pkgid, unsigned char **out, size_t *len)
{
    unsigned char salt[32];

    memset(salt, 0xFF, sizeof(salt));

    unsigned char *duk = (unsigned char *)malloc(sizeof(unsigned char) * (DUK_LEN + 1));
    if (duk == NULL) {
        WAE_SLOGE("Failed to allocate memory for duk.");
